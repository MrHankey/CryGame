...
const char* __get_c_string( const char* __str )
{
	return __str;
}

__Named_exception::__Named_exception(const char* __str) {
  ...
  strncpy(_M_name, __get_c_string(__str), __size - 1);
  _M_name[__size - 1] = '\0';
#else
  strncpy_s(_M_name, __size, __get_c_string(__str), __size - 1);
#endif
}
