...
#  include <stl/memstat.h>
#endif

// Jan 31, 2009 Chris Raine: 
// crycg workaround - we need to define the index operators of vectors
// always_inline to prevent possible cross page calls returning
// references (not supported at the moment)
#if !defined(ILINE) 
	#if defined(__SPU__)
...
public:
  ...
  _Vector_base(const _Alloc& __a)
    : _M_start(0), _M_finish(0), _M_end_of_storage(__a, 0) {
			MEMSTAT_REGISTER_CONTAINER(this, EMemStatContainerType::MSC_Vector, _Tp);
	}
  ...
  _Vector_base(size_t __n, const _Alloc& __a)
    ...
    _STLP_MPWFIX_TRY _STLP_MPWFIX_CATCH
		...
		MEMSTAT_REGISTER_CONTAINER(this, EMemStatContainerType::MSC_Vector, _Tp);
		MEMSTAT_BIND_TO_CONTAINER(this, this->_M_start);
		MEMSTAT_USAGE(this->_M_start, reinterpret_cast<char*>(this->_M_finish) - reinterpret_cast<char*>(this->_M_start));
  }
...
private:
  ...
  void _M_initialize_aux(_Integer __n, _Integer __val,
    ...
    size_type __real_n = __n;
		pointer newStart = this->_M_end_of_storage.allocate(__n, __real_n);

		MEMSTAT_REBIND_TO_CONTAINER(this, this->_M_start, newStart);
...
#  endif /* _STLP_NEEDS_EXTRA_TEMPLATE_CONSTRUCTORS */
  ...
  void swap(_Self& __x) {
		...
		MEMSTAT_SWAP_CONTAINERS(this, &__x);
  }
...
private:
  ...
  void _M_set(pointer __s, pointer __f, pointer __e) {
		MEMSTAT_REBIND_TO_CONTAINER(this, this->_M_start, __s);
  ...
  void _M_range_initialize(_ForwardIterator __first, _ForwardIterator __last,
                           const forward_iterator_tag &) {
    size_type __n = _STLP_STD::distance(__first, __last);
		pointer newStart = this->_M_end_of_storage.allocate(__n, __n);
