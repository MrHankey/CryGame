...
#ifdef CRY_MEMORY_ALLOCATOR
#define DEFAULT_POOL_SIZE 524288
#endif
...
#if defined(PS3)
	static void* _STLP_CALL allocate(size_t __n, size_t alignment) { return __stl_new(__n, alignment); }
#endif
...
class _STLP_CLASS_DECLSPEC __node_alloc1 {
...
#if defined(PS3)
	/* __n must be > 0      */
	static void* _STLP_CALL allocate(size_t& __n, size_t alignment) 
	{ return (__n > (size_t)_MAX_BYTES) ?  __stl_new(__n, alignment) : _M_allocate(__n); }
#endif //PS3
	/* __p may not be 0 */
...
//#  endif
#ifndef CRY_MEMORY_ALLOCATOR
_STLP_EXPORT_TEMPLATE_CLASS __node_alloc<_STLP_NODE_ALLOCATOR_THREADS, 0>;
#else
_STLP_EXPORT_TEMPLATE_CLASS node_alloc<_STLP_NODE_ALLOCATOR_THREADS, DEFAULT_POOL_SIZE>;
#endif /*CRY_MEMORY_ALLOCATOR*/

#endif /* _STLP_USE_TEMPLATE_EXPORT */
#ifndef CRY_MEMORY_ALLOCATOR
typedef __node_alloc<_STLP_NODE_ALLOCATOR_THREADS, 0> _Node_alloc;
#else
typedef node_alloc<eCryMallocCryFreeAll, _STLP_NODE_ALLOCATOR_THREADS, DEFAULT_POOL_SIZE> _Node_alloc;
#endif /*CRY_MEMORY_ALLOCATOR*/
...
typedef __sgi_alloc  __multithreaded_alloc;
#else
typedef node_alloc<eCryMallocCryFreeAll, false, DEFAULT_POOL_SIZE> __single_client_alloc;
typedef node_alloc<eCryMallocCryFreeAll, true, DEFAULT_POOL_SIZE>  __multithreaded_alloc;
#endif
#endif
...
public:
...
#if defined(PS3)	    
			_Tp* __ret = __REINTERPRET_CAST(_Tp*, __sgi_alloc::allocate(__buf_size, __alignof__(value_type)));
#else
	...
	void cleanup() 
	{
#if defined (CRY_MEMORY_ALLOCATOR) && (defined USE_NODE_ALLOC)
		__sgi_alloc::cleanup(); 
#endif // _DEBUG
	}
	...
	{
#if defined (CRY_MEMORY_ALLOCATOR) && (defined USE_NODE_ALLOC)
		return __sgi_alloc::get_heap_size(); 
#else 
		return 0;
#endif // _DEBUG
	}

	size_t get_wasted_in_allocation() 
	{
#if defined (CRY_MEMORY_ALLOCATOR) && (defined USE_NODE_ALLOC)
		return __sgi_alloc::get_wasted_in_allocation(); 
#else 
		return 0;
#endif // _DEBUG
	}

	size_t get_wasted_in_blocks() 
	{
#if defined (CRY_MEMORY_ALLOCATOR) && (defined USE_NODE_ALLOC)
		return __sgi_alloc::get_wasted_in_blocks(); 
#else 
		return 0;
#endif // _DEBUG
	}
