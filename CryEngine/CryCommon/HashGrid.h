...
struct hash_grid_no_position
{
	...
	{
		switch(0) { case 0: case 0: "hash_grid query performed without a valid position-retriever implementation"; };
...
public:
	...
	{
		size_t index = hash(key) % CellCount;
	...
	{
		size_t index = hash(to) % CellCount;
	...
	uint32 query_sphere(const key_type& center, const key_value& radius, Container& container) const
	{
		...
		if (!empty())
		{
			...
			discreet_type mind = discreet(minc);
			discreet_type maxd = discreet(maxc);
			...
			for ( ; current[0] <= maxd[0]; ++current[0])
			{
				for ( ; current[1] <= maxd[1]; ++current[1])
				{
					...
					{
						size_t hash_value = hash(current);
						size_t index = hash_value % CellCount;
