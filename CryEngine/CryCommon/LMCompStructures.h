...
struct LMGenParam
{
	...
	uint32 m_iTextureResolution;		//!< Resolution of the produced textures
	float m_fTexelSize;				//!< World space size of one texel
	uint32 m_iSubSampling;			//!< Amount of sub-sampling used for shadows etc.
	uint32 m_iMinBlockSize;			//!< Smallest possible block assigned on a lightmap
	uint32 m_uiSmoothingAngle;		//!< smoothing angle where edge sharing triangles be treated as smooth even when the normal is not smoothed and normal generation gets smoothed to
