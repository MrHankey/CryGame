...
void id_map<IDType, ValueType, IndexType, CounterType>::grow(size_t amount)
{
	if (amount)
	{
		frees_.reserve(frees_.capacity() + amount);
...
{
	return values_.size() - frees_.size();
}
...
{
	return frees_.size() == frees_.capacity();
}
